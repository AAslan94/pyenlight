...
NODE_SWEEP = [50, 100, 150, 200 ,300, 350,400,450,500]
...
def make_synthetic_dto(n, hidden_fraction=0.0):
    n_hidden = int(round(hidden_fraction * n))
    hidden_idx = np.arange(n_hidden)
    ...
    if n_hidden > 0:
        snr_d[hidden_idx] = LOW_SNR_DB
        snr_u[hidden_idx, 0] = LOW_SNR_DB
...
def make_node_dataframe(em, case_name, n, mac_result=None):
    df = pd.DataFrame({
...
def summarize_case(em, case_name, n, mac_result=None):
    row = {
        "case": case_name,
        "N": n,
...
def run_case(n, case_name, mac_enabled, btma_mode, hidden_fraction):
    config = EnLightConfig()
    design = prepare_design(master_design_example, n)
    dto = make_synthetic_dto(n, hidden_fraction=hidden_fraction)
    ...
    summary_row = summarize_case(em, case_name, n, mac_result=mac_result)
    node_df = make_node_dataframe(em, case_name, n, mac_result=mac_result)
...
def main():
    ...
    cases = [
        ...
        {
            "case_name":  r"CSMA/CA ($10\%$ hidden nodes)",
            "mac_enabled": True,
            "btma_mode": True,
    ...
    for n in NODE_SWEEP:
        ...
        for case in cases:
            ...
            row, node_df = run_case(
                ...
                hidden_fraction=case["hidden_fraction"],
            )
